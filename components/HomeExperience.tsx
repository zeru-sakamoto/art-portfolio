...
export default function HomeExperience({ featured, catalog }: HomeExperienceProps) {
        ...
        () => {
            ...
            mm.add(
                ...
                (context) => {
                    ...
                    const heroStage = heroRef.current?.querySelector(".hero-stage") ?? null;
                    const catalogStage =
                        catalogRef.current?.querySelector(".catalog-stage") ?? null;
                    const collageItems = heroRef.current
                        ? gsap.utils.toArray<HTMLElement>(".hero-collage-item", heroRef.current)
                        : [];
                    const catalogCards = catalogRef.current
                        ? gsap.utils.toArray<HTMLElement>(".highlight-card", catalogRef.current)
                        : [];
                    ...
                    if (reduceMotion) {
                        ...
                        gsap.set(catalogStage, { opacity: 1, scale: 1, yPercent: 0, filter: "none" });
                    ...
                    if (isDesktop) {
                        ...
                        collageItems.forEach((item, i) => {
                            const { xPercent, yPercent } = collageDolly[i % collageDolly.length];
                            tl.fromTo(
                                item,
    ...
    return (
        <>
            ...
            <ArtworkLightbox artwork={selectedArtwork} onClose={() => setSelectedArtwork(null)} />
        </>
    );
}
